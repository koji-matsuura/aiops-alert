...
Action Groups の構成：
- **FunctionSchema**: 各アクションの入出力スキーマを定義
- **Executor.Lambda**: Lambda 関数の ARN を指定
- **Description**: アクションの説明

Lambda 関数（`lib/lambda_handler.py`）が messageVersion 1.0 形式のリクエストを受け取り、以下を実行：
- 関数名（function フィールド）を解析
- 対応する FR-01～FR-06 ハンドラを呼び出し
- 結果を messageVersion 1.0 形式で Bedrock Agent に返す

> **重要**：Lambda 関数は単一の `handler()` エントリポイントで全アクションを処理。Bedrock Agent はアクションの判定・パラメータ設定を実施。
